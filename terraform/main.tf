...
resource "aws_api_gateway_rest_api" "test-gateway" {
  name = "hello-gateway"
  description = "Gateway for hello Lambda function"
}

resource "aws_api_gateway_resource" "hello" {
  rest_api_id = aws_api_gateway_rest_api.test-gateway.id
  parent_id = aws_api_gateway_rest_api.test-gateway.root_resource_id
  path_part = "hello"
}
...
resource "aws_api_gateway_method_response" "response_200" {
  ...
  depends_on = [aws_api_gateway_rest_api.test-gateway]
}
...
resource "aws_lambda_permission" "gateway_lambda_permission" {
  ...
  source_arn = "${aws_api_gateway_rest_api.test-gateway.execution_arn}/*/*/hello"
}
...
resource "null_resource" "replace-host" {
  provisioner "local-exec" {
    command = "sed 's#REPLACE_URL#${aws_api_gateway_deployment.test_deployment.invoke_url}/hello#' ../frontend/index.html > ../frontend/index.local.html"
  }
}

output "url" {
  value = "http://${aws_s3_bucket.test_bucket.website_endpoint}"
}